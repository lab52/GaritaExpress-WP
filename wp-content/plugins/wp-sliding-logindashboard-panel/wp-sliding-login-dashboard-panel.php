	...
	<div id="panel" <?php  if ($wp_sliding_panel_replace_default) echo 'style="height: '.$wp_sliding_panel_li_height.'px"'; ?>>
		<div class="content clearfix">
			...
			<div class="left border">
				<h1><?php _e('Bienvenid@'); ?> <?php echo $user_identity ?></h1>
				<ul>					
			...
			<div class="left narrow">			
				...
				<ul>					
					<li><a href="<?php bloginfo('url') ?>/wp-admin/index.php"><?php _e('Dashboard') ?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/profile.php"><?php _e('Perfil')?></a></li>
					<?php if ( current_user_can('level_1') ) : ?>
						<li><a href="<?php bloginfo('url') ?>/wp-admin/edit-comments.php"><?php _e('Comentarios')?></a></li>
					<?php endif ?>
	        		<li><a href="<?php echo wp_logout_url(get_permalink()); ?>" rel="nofollow" title="<?php _e('Log out'); ?>"><?php _e('Cerrar sesión'); ?></a></li>
				</ul>	
				<?php if ( current_user_can('level_10') ) : ?>		
				<h2><?php _e('Appearance')?></h2>
				<ul>						
					<li><a href="<?php bloginfo('url') ?>/wp-admin/themes.php"><?php _e('Temas')?></a></li>
			...
			<div class="left narrow">			
				<h2><?php _e('Post')?></h2>				
				<ul>					
					<li><a href="<?php bloginfo('url') ?>/wp-admin/post-new.php"><?php _e('Nuevo Post')?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/edit.php"><?php _e('Editar Posts')?></a></li>
				<?php if ( current_user_can('level_3') ) : ?>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/edit-tags.php"><?php _e('Tags')?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/categories.php"><?php _e('Categorías')?></a></li>
				<?php endif ?>
				</ul>
				<?php if ( current_user_can('level_10') ) : ?>		
				...
				<ul>						
					...
					<li><a href="<?php bloginfo('url') ?>/wp-admin/plugin-install.php"><?php _e('Instalados')?></a></li>
			...
			<div class="left narrow">
				...
				<h2><?php _e('Pages')?></h2>				
				<ul>		
					<li><a href="<?php bloginfo('url') ?>/wp-admin/post-new.php"><?php _e('Nueva Página')?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/edit-pages.php"><?php _e('Editar')?></a></li>
				</ul>
				<?php endif ?>			
				<h2><?php _e('Media')?></h2>				
				<ul>					
					<li><a href="<?php bloginfo('url') ?>/wp-admin/upload.php"><?php _e('Librería')?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/media-new.php"><?php _e('Agregar Nuevo')?></a></li>
				</ul>
				<?php if ( current_user_can('level_3') ) : ?>		
				<h2><?php _e('Users')?></h2>
				<ul>						
					<li><a href="<?php bloginfo('url') ?>/wp-admin/users.php"><?php _e('Autores &amp; Usuarios')?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/user-new.php"><?php _e('Agregar Nuevo')?></a></li>
				</ul>
				<?php endif ?>
			</div>
			<?php endif ?>
			...
			<div class="left narrow">			
				...
				<ul>						
					...
					<li><a href="<?php bloginfo('url') ?>/wp-admin/options-writing.php"><?php _e('Escribir')?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/options-reading.php"><?php _e('Leer')?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/options-discussion.php"><?php _e('Discusiones')?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/options-media.php"><?php _e('Media')?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/options-privacy.php"><?php _e('Privacidad')?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/options-permalink.php"><?php _e('Permalinks')?></a></li>
					<li><a href="<?php bloginfo('url') ?>/wp-admin/options-misc.php"><?php _e('Misceláneo')?></a></li>
				</ul>
			</div>
			<?php endif ?>
                <?php } ?>
...
<!--	        <li><a href="<?php echo wp_logout_url(get_permalink()); ?>" rel="nofollow" title="<?php _e('Log out'); ?>"><?php _e('Log out'); ?></a></li>
			<li class="sep">|</li>-->
			<li id="toggle">
				<a id="open" class="open" href="#"><?php if ($wp_sliding_panel_replace_default) echo $wp_sliding_panel_li_open_label; else _e('Mi cuenta');?></a>
				<a id="close" style="display: none;" class="close" href="#"><?php if ($wp_sliding_panel_replace_default) echo $wp_sliding_panel_li_close_label; _e('Cerrar');?></a>
			</li>
	    	<li class="right">&nbsp;</li>
		</ul> 
	</div> <!-- / top -->
...
?>
	<div id="panel" <?php  if ($wp_sliding_panel_replace_default) echo 'style="height: '.$wp_sliding_panel_lo_height.'px"'; ?>>
		<div class="content clearfix">
                    <?php if($wp_sliding_panel_replace_default) {
                        ...
                        ?>
			<div class="left border">
				<h1>¡<?php _e('Bienvenido a ') ?><?php bloginfo('name') ?>!</h1>
			</div>
			<div class="left">
				<!-- Login Form -->
				<form class="clearfix" action="<?php bloginfo('url') ?>/wp-login.php" method="post">
					<h1><?php _e('Inicia Sesión') ?></h1>
					<label class="grey" for="log"><?php _e('Ususario:') ?></label>
					<input class="field" type="text" name="log" id="log" value="<?php echo wp_specialchars(stripslashes($user_login), 1) ?>" size="23" />
					<label class="grey" for="pwd"><?php _e('Contraseña:') ?></label>
					<input class="field" type="password" name="pwd" id="pwd" size="23" />
	            	<label><input name="rememberme" id="rememberme" type="checkbox" checked="checked" value="forever" />&nbsp;<?php _e('Recuérdame') ?></label>
        			<div class="clear"></div>
					<input type="submit" name="submit" value="Entrar" class="bt_login" />
					<input type="hidden" name="redirect_to" value="<?php echo $_SERVER['REQUEST_URI']; ?>"/>
					<a class="lost-pwd" href="<?php bloginfo('url') ?>/wp-login.php?action=lostpassword"><?php _e('¿Olvidaste tu contraseña?') ?></a>
				</form>
			</div>
			...
			<?php if (get_option('users_can_register')) : ?>
				...
				<form name="registerform" id="registerform" action="<?php echo site_url('wp-login.php?action=register', 'login_post') ?>" method="post">
					<h1><?php _e('Regístrate') ?></h1>
					<label class="grey" for="user_login"><?php _e('Usuario') ?></label>
					...
					<label id="reg_passmail"><?php _e('Se te enviará una contraseña a tu e-mail.') ?></label>
					<input type="submit" name="wp-submit" id="wp-submit" value="<?php _e('Registrarse'); ?>" class="bt_register" />
				</form>
			<?php else : ?>
				<h1><?php _e('Regístrate') ?></h1>
				<p><?php _e('Por favor contacta al administrador.') ?></p>
			<?php endif ?>
			</div>
                    <?php } ?>
		</div>
	...
	<div class="tab">
		<ul class="login">
	    	...
	    	<!-- Login / Register -->
			<li id="toggle">
				<a id="open" class="open" href="#"><?php  if ($wp_sliding_panel_replace_default) echo $wp_sliding_panel_lo_open_label; else _e('Mi Cuenta'); ?></a>
				<a id="close" style="display: none;" class="close" href="#"><?php  if ($wp_sliding_panel_replace_default) echo $wp_sliding_panel_lo_close_label; else _e('Cerrar'); ?></a>
			</li>
	    	<li class="right">&nbsp;</li>
		</ul>
	</div> <!-- / top -->
